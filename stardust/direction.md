...
## Named deviation — 404 page geometry probe (2026-09-21)

`stardust/scripts/replica/lift.mjs` refuses `/en/home/404.html` at both widths (exit 5, `INVALID CAPTURE — suspect: 133 chars of text, near-empty`; `live-session.mjs` CAPTURE_FLOOR.emptyLen = 400 has no override flag), and `stitch-shot.mjs` applies the same floor on the live side. The page is legitimately near-empty (badge, one heading, one CTA). Replacement: `stardust/.work/probes/live-rects.mjs` — one live navigation per width, element rects + the same computed-style keys the lift records, written to `stardust/replica/capture/lift/en-home-404-html-{1440,360}.probe.json`. It replaces lift.mjs Phase 3 evidence for this page only; the pixel gate is attempted with the shipped `gate.sh` and its verdict (or refusal) is recorded as-is. Plugin gap ledgered in `stardust/learnings.md`: a `--allow-short` / `--floor` control on the capture sanity floor for legitimately thin pages (error pages, confirmation pages).
